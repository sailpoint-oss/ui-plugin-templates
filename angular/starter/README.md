...
A standalone Angular app that SailPoint Identity Security Cloud (ISC) loads inside a sandboxed iframe. It communicates with the ISC host through the SailPoint UI Plugin SDK (a cross-origin `postMessage` protocol). The starter comes prewired with that SDK.
